...
async function run(args) {
  ...
  await upload(files, dir, args.output, config, { timeout })
}

async function upload(files, from, to, config, opts) {
  ...
  for (const file of files) {
    ...
    // eslint-disable-next-line
    const res = await store.put(dist, fs.createReadStream(path), { timeout: opts.timeout })
